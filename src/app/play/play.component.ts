...
export class PlayComponent implements OnInit {
  ...
  submitQuote(e: MouseEvent, quote: Quote, i: number) {
    ...
    this.http
      .post(this.game.apiRoot + "/game/room/quotes", data)
      .subscribe(res => {
  ...
  chooseQuote(e: MouseEvent, i: number) {
    e.preventDefault();
    this.http
      .post(this.game.apiRoot + "/game/room/quotes/choose", { i: i })
      .subscribe(res => {});
  }
  ...
  showFBPictures(e: MouseEvent) {
    ...
    FB.api("/me/photos?fields=album,picture,images", (response: any) => {
      ...
      this.fbImages = response.data.map((x: any) => ({
        id: x.id,
        src: x.picture,
        link: x.images[0].source
      }));
    });
  }
}
